...
class Tree {
 ...
 public:
  ...
  Tree(const std::string& s) {
    ...
    std::function<unsigned int(const std::string&, unsigned int)> findComma =
        [](const std::string& str, unsigned int sta) {
          while (sta < str.size() && str[sta] != ',') ++sta;
          return sta;
        };

    std::function<unsigned int(const std::string&, unsigned int)> findLetter =
        [](const std::string& str, unsigned int sta) {
    ...
    std::function<void(const std::string&)> error =
        [](const std::string& func) {
          std::cerr << "Error: the number of parameters for " << func
